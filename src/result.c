...
result_to_str(result_t r)
{
	...
	case ERR_SEMA_INIT_COMPOUND_EXCESS_ELEMENTS:
		s = strdup("Excess elements in compound initializer");
		break;
	...
	case ERR_SEMA_INIT_STR_LITERAL_INVALID:
		...
		break;
	case ERR_SEMA_INIT_STRUCT_MISMATCH:
		s = strdup("Incompatible struct types in variable declaration "
		           "and initializer");
		break;
	...
	case ERR_SEMA_LABEL_FOLLOWED_BY_DECLARATION:
		s = my_asprintf("Label %s followed by variable or struct "
